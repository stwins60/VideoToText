...
try:
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS details (id INTEGER PRIMARY KEY AUTOINCREMENT, details TEXT, size INTEGER, upload_type TEXT, 
                audio_id INTEGER, FOREIGN KEY(audio_id) REFERENCES audio(id),
                texts_id INTEGER, FOREIGN KEY(texts_id) REFERENCES texts(id),
                video_id TEXT, FOREIGN KEY(video_id) REFERENCES video(id))""")
